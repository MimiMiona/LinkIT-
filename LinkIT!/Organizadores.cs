    ...
    public partial class Organizadores : Form
    {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                if (fila.Cells["idOrganizador"].Value == null ||
                    fila.Cells["idOrganizador"].Value == DBNull.Value)
                        return;

                idSeleccionado = Convert.ToInt32(fila.Cells["IdOrganizador"].Value);

                textNombre.Text = fila.Cells[1].Value.ToString();
                textApellido.Text = fila.Cells[2].Value.ToString();
                textDNI.Text = fila.Cells[3].Value.ToString();
                textTelefono.Text = fila.Cells[4].Value.ToString();
            }
        }
